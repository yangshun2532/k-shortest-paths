...
namespace asu_emit_qyan
{
	...
	public:
		...
		CQYDirectedGraph(const std::string& input_file_name); // deprecated!!!
		CQYDirectedGraph(const std::string& input_file_name, bool is_full_graph);
		...
		void PrintOut(const std::string& out_file_name);
		void PrintOut(std::ostream& out_stream);
		void PrintOutAllEdges(std::ostream& os);
