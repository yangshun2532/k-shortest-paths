...
{	
	...
	public:
		...
		{
			out_stream << m_dCost << "	" << m_vVertexList.size() - 1 << "	";
			std::copy(m_vVertexList.begin(), m_vVertexList.end(), std::ostream_iterator<int>(out_stream, " "));
			out_stream << std::endl;
// 			out_stream << "Cost: " << m_dCost 
// 				<< " Length: " << m_vVertexList.size() - 1
// 				<< std::endl;
...
// 			out_stream << std::endl <<  "*********************************************" << std::endl;	
	...
	private: // members
		...
		int m_nId;
		double m_dCost;  
		std::vector<int> m_vVertexList; // record the nodes through the path
